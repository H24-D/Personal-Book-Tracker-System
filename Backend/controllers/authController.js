...
async function login(req, res, next) {
  try {
    ...
    console.log("[Auth] Login successful — token expires in 2 days");
    res.json({ token });
  } catch (err) {
...
async function register(req, res, next) {
  try {
    ...
    if (!usernameRegex.test(username)) return res.status(400).json({ message: "Username must start with a letter and can only contain letters, numbers, and # _ ! characters" });
    ...
    if (!email.toLowerCase().endsWith("@gmail.com")) return res.status(400).json({ message: "Email must be a valid @gmail.com address" });

    const mobileRegex = /^\d{10}$/;
    if (!mobileRegex.test(mobile)) return res.status(400).json({ message: "Mobile number must be exactly 10 digits" });
...
async function forgotPassword(req, res) {
  try {
    ...
    if (!normalizedEmail.endsWith("@gmail.com")) return res.status(400).json({ message: "Please enter a valid @gmail.com address" });

    const pool = db.getPool();
    await ensureTokenTable(pool);

    const [rows] = await pool.query("SELECT id, email FROM users WHERE LOWER(email) = ? LIMIT 1", [normalizedEmail]);
    ...
    await sendBrevoEmail({
      ...
      html: `
        <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;max-width:480px;margin:0 auto;background:#09090b;border:1px solid #27272a;border-radius:16px;overflow:hidden;">
          ...
          <div style="padding:36px 32px;">
            ...
            <p style="color:#a1a1aa;font-size:15px;line-height:1.6;margin:0 0 28px;">
              Click the button below to reset your password. This link expires in <strong style="color:#e4e4e7;">1 hour</strong>.
            </p>
            <div style="text-align:center;margin-bottom:28px;">
              <a href="${resetLink}" style="display:inline-block;padding:14px 32px;background:linear-gradient(135deg,#0ea5e9,#0284c7);color:#fff;text-decoration:none;font-weight:700;font-size:16px;border-radius:10px;">Reset Password</a>
            </div>
            <p style="color:#71717a;font-size:13px;margin:0 0 8px;">Or copy this link:</p>
            <p style="word-break:break-all;margin:0 0 24px;"><a href="${resetLink}" style="color:#0ea5e9;font-size:13px;">${resetLink}</a></p>
            <hr style="border:none;border-top:1px solid #27272a;margin:0 0 20px;" />
            <p style="color:#52525b;font-size:13px;margin:0;">If you didn't request this, you can safely ignore this email.</p>
          </div>
          <div style="padding:20px 32px;background:#18181b;text-align:center;">
            <p style="color:#52525b;font-size:12px;margin:0;">© ${new Date().getFullYear()} Personal Book Tracker</p>
          </div>
        </div>
      `,
    });
...
async function verifyResetToken(req, res) {
  try {
    ...
    const [rows] = await pool.query("SELECT id, expires_at, used FROM password_reset_tokens WHERE token = ? LIMIT 1", [token]);
...
async function resetPassword(req, res) {
  try {
    ...
    const [rows] = await pool.query("SELECT id, user_id, expires_at, used FROM password_reset_tokens WHERE token = ? LIMIT 1", [token]);
    if (!rows || rows.length === 0) return res.status(400).json({ message: "Invalid or expired reset link." });

    const row = rows[0];
    const expired = new Date(row.expires_at) < new Date();
    if (row.used || expired) return res.status(400).json({ message: "This reset link has already been used or has expired." });
...
async function updateBook(req, res) {
  try {
    ...
    // Merge: keep existing values for fields not sent in request
...
module.exports = {
  ...
  verifyResetToken, resetPassword, validatePassword,
